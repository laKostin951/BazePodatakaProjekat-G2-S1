...
from ui.popUp.popup import Error, Info, Success
...
class SQFile(File, ABC):
    def __init__(self, path_c, auto_load=False, metadata_c=None, structure_dock=None):  # path_c = Path 'Class'(path of file
        super().__init__(path_c, auto_load, metadata_c, structure_dock)
    ...
    def get_relation_index_by_path(self, path_c, relative="parent_relation"):
        for index, relation in enumerate(self.metadata_c.metadata["sequential_info"][relative]):
            if path_c.is_same(relation["path_of_child_table"]):
                return index
